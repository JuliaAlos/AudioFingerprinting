...
if __name__ == '__main__':
  ...
  def grouper(iterable, n, fillvalue=None):
    ...
    return (filter(None, values) for values in zip_longest(*args, fillvalue=fillvalue))
  ...
  def return_matches(hashes):
    ...
    for hash_value, offset in hashes:
        mapper[hash_value.upper()] = offset  # Store offsets in the mapper
    values = list(mapper.keys())  # Convert keys to a list for multiple uses

    for split_values in grouper(values, 1000):
        # Convert split_values to a list to ensure it has a length
        ...
        if matches_found > 0:
            ...
            msg = '   ** no matches found (step %d/%d)'  # Corrected the message
