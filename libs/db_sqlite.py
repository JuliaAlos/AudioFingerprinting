...
class SqliteDatabase(Database):
  ...
  def insert(self, table, params):
    ...
    placeholders = ', '.join(['?'] * len(params))  # Create placeholders based on the number of params
    values = list(params.values())  # Convert to list to ensure it's the correct type

    query = "INSERT INTO %s (%s) VALUES (%s)" % (table, keys, placeholders)
