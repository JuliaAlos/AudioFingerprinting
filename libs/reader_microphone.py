...
class MicrophoneReader(BaseReader):
  ...
  def save_recorded(self, output_filename):
    ...
    # Calculate chunk_length as an integer
    chunk_length = len(self.data[0]) // self.channels  # Use integer division

    # Reshape the data for saving
    result = numpy.reshape(self.data[0], (chunk_length, self.channels))

    # Convert the result to bytes before writing
    wf.writeframes(result.tobytes())  # Use .tobytes() to convert to bytes
    wf.close()
